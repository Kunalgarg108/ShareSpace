...
export const login = async (req, res) => {
  try {
    ...
    return res.cookie('token', token, { httpOnly: true, sameSite: 'Strict', maxAge: 24 * 60 * 60 * 1000 , secure : true}).status(200).json({ success: true, message: "Login successful", user });
  } catch (error) {
    res.status(500).json({ success: false, message: "Server error", error: error.message });
  }
}
...
export const logout = async (req, res) => {
  try {
    res.clearCookie('token', { httpOnly: true, sameSite: 'Strict', maxAge: 0 , secure : true });
